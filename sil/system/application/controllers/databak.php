...
class Data extends Controller {
	...
	function get_laporan($year,$type){
       if (Current_User::user()){if(Current_User::user()->role != 2) redirect("error/403");}else redirect("error/403");
		...
		// save data_tahun yang sesuai ke array res_data_tahun
		$res_data_tahun = array();
		foreach($row as $item){
			array_push($res_data_tahun,$item['id']);
		}
		
		// cari offset nya untuk limit
		$count = count($row);
		if( $count >0 ) {
			$total_pages = ceil($count/$limit);
		} else {
		...
		// get all $type data
		// $q = Doctrine::getTable($type)->findAll();
		$q = Doctrine::getTable($type)->findAll()->toArray();
		// $this->infolib->view_r($q);
		// $this->infolib->view_r($res_data_tahun);
		
			// if(in_array(81,$res_data_tahun)){
				// echo 'ok';
			// }
		$responce->page = $page;
		$responce->total = $total_pages;
		...
		foreach($q as $row){
			// check if data_tahun exist in res_data_tahun
			if(in_array($row->data_tahun,$res_data_tahun)){
				echo 'masuk';
				// check if data produced in limit offset config
				if($iterator >= $start && $iterator <= $limit){
					$kota = Doctrine::getTable('user')->find($row->kota_id);
					
					$responce->rows[$row_id]['id']=$row->id;
					...
					$responce->rows[$row_id]['cell'] = $data_rows;
					//// end of row ////
